        ...
        if(!empty($preValidation)) {
            ...
            $validation =
                (request('file_type') == 'image')
                    ? ['required', 'mimes:' . config('attachment.image_valid_mimes'), 'max:' . (config('attachment.image_maximum_size') * 1024), 'attachment_check_disk_is_public', 'attachment_disk_not_found']
                    : ( (request('file_type') == 'video')
                    ? ['required', 'mimes:' . config('attachment.video_valid_mimes'), 'max:' . (config('attachment.video_maximum_size') * 1024), 'attachment_check_disk_is_public', 'attachment_disk_not_found']
                    : ['required', 'mimes:' . config('attachment.attachment_valid_mimes'), 'max:' . (config('attachment.attachment_maximum_size') * 1024), 'attachment_disk_not_found'] );
        }
        return [
            'file' => $validation,
